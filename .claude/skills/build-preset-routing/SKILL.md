...
- **Read / verify:** `get_current_preset`, `cpu_load(detail)`, `get_io_settings`,
  `current_preset_position`, **`output_meter(hold_s, detail)`** (per-port peak-hold +
  limiter flags — the fastest check for "is signal reaching the outputs / is it
  clipping"). With the `.[audio]` extra: `measure_loudness()`, and `suggest_levels()`
  to compare presets by loudness without writing anything (docs/LEVELING.md). The only non-MCP helper is `tools/gui/gui.py shot` for a
